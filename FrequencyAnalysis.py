...
class Attack:
    def __init__(self):
        ...
        self.freq_precalculated = {'a': 0.0817, 'b': 0.0150, 'c': 0.0278, 'd': 0.0425, 'e': 0.1270, 'f': 0.0223,
    ...
    def calculate_freq(self, cipher):
        ...
        #------
        for c in cipher:  #loop through the cipher
            if c in self.freq:  #if the char is found in the frequency dict
                self.freq[c] += 1
                letter_count += 1
        #------
    ...
    def calculate_matches(self):
        for cipher_char in self.alphabet:
            ...
            #loop through the frquency analysis found and the pre-calculated one
            # log the difference found, and then we sort the dictionary where the lowest
            # indexes will be the letters which most closely matches the pre-calculated frequency
            for plain_char in self.alphabet:
                map[plain_char] = round( 
                  abs(  self.freq[cipher_char] - self.freq_precalculated[plain_char]  ),
                  4)
    ...
    def guess_key(self):
        ...
        #loop through the cipher chars left
        for cipher_char in self.cipher_chars_left:

            #pick the one that supposedly is the closest match
            # e.g.: cipher_char = 'a' , plain_char = 'k', diff = 0.0
...
#----------------------------
# key guesses should be inserted here before the system tries to match them
